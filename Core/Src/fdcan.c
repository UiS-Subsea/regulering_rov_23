...
void CANFD_Init(void) {
  ...
  sFilterConfig.FilterID1 = FD_FILTER1_START;
  sFilterConfig.FilterID2 = FD_FILTER1_SLUTT;
  ...
  /*Id-er innenfor ID1 og ID2 slippes gjennom*/
  sFilterConfig.FilterID1 = FD_FILTER2_START;
  sFilterConfig.FilterID2 = FD_FILTER2_SLUTT;
  if (HAL_FDCAN_ConfigFilter(&hfdcan1, &sFilterConfig) != HAL_OK){
    Error_Handler();
  }
...
//   * MessageMarker: Makør for melding (satt til adresse)*/
...
uint32_t getDataFieldLength(uint8_t bytes) {
  switch (bytes) {
...
int SendDataNew(uint16_t adr, void* data, uint8_t size)
{
  ...
  uint32_t data_frame_len = getDataFieldLength(size);
  ...
  TxHeader.IdType = FDCAN_STANDARD_ID;
  TxHeader.TxFrameType = FDCAN_DATA_FRAME;
  TxHeader.DataLength = data_frame_len; // FDCAN_DLC_BYTES_1
  TxHeader.ErrorStateIndicator = FDCAN_ESI_ACTIVE;
  TxHeader.BitRateSwitch = FDCAN_BRS_ON;
  TxHeader.FDFormat = FDCAN_FD_CAN;
  TxHeader.TxEventFifoControl = FDCAN_STORE_TX_EVENTS;
...
//  {
//    uint8_t val = ((uint8_t*)txbuffer)[i];
//    print("%X",val);
//  }
//  print("\r\n");

  /*Legger til meldingen i Tx buffer, Returnerer 0 hvis error*/
  if (HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &TxHeader, (uint8_t*)txbuffer) != HAL_OK)
  {
    return 0;
  }
  /* Stopper Malloc-ed variabler fra å lekke minne og returnerer 1 (melding sendt)*/
...
{
  int status = SendDataNew(REG_SEND_STATUS, data, 1);
  if(status != 1) print("fdcanerr: %d\r\n",status);
}
...
int hexToValues(void* voidData, const uint8_t* hexData, uint8_t sizeBytes, DataType dataType){
  ...
  switch(dataType) {
    ...
    case uint16:
      ...
      break;
    case float32:
      /* Byte størrelsen kjekk: 6%4=2 -> returnerer 3
